...
In addition, you can configure Container Runtime Policies to provide real-time protection of containers against malware. For more information, refer to [Real-time Malware Protection](./container-runtime-policy-components#section-real-time-malware-protection).
...
However, the Default runtime policy can include the optional Bypass Scope control, which defines the containers to which this policy will not apply. See [Controls](./container-runtime-policy-components#section-controls).
